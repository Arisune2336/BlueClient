...
namespace AuthGG
{
    ...
    internal class OnProgramStart
    {
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            {
                MessageBox.Show("Lütfen Tüm Alanları Doldurun", Name);
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show("Olası kötü niyetli faaliyet tespit edildi!", OnProgramStart.Name);
                        Process.GetCurrentProcess().Kill();
                    }
                    ...
                    {
                        MessageBox.Show("Güvenlik hatası tetiklendi!", Name);
                        Process.GetCurrentProcess().Kill();
                    }
                    switch (response[2])
                    {
                        case "success":
                            ...
                            else
                            {
                                ...
                                if (response[12] == "Enabled")
                                {
                                    ...
                                    {
                                        MessageBox.Show($"Dosyada değişiklik yapıldı, bütünlük doğrulanamadı!", Name);
                                        Process.GetCurrentProcess().Kill();
                                    }
                                }
                            }
                        ...
                        case "binderror":
                            ...
                            return;
                        case "banned":
                            MessageBox.Show("Bu uygulama TOS'u ihlal ettiği için yasaklandı" + Environment.NewLine + "DemirPremiumla İletişime geçin", Name);
                            Process.GetCurrentProcess().Kill();
                            return;
                    }
                    Security.End();
    ...
    internal class API
    {
        public static void Log(string username, string action)
        {
            ...
            {
                MessageBox.Show("Lütfen önce uygulamanızı başlatın", OnProgramStart.Name);
                Process.GetCurrentProcess().Kill();
            }
            if (string.IsNullOrWhiteSpace(action))
            {
                MessageBox.Show("Eksik günlük bilgileri!", ApplicationSettings.Name);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
        ...
        public static bool AIOLogin(string AIO)
        {
            ...
            {
                MessageBox.Show("Lütfen Tüm Alanları Doldurun", ApplicationSettings.Name);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    if (response[0] != Constants.Token)
                    {
                        MessageBox.Show("Güvenlik hatası tetiklendi", OnProgramStart.Name);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Security.MaliciousCheck(response[1]))
                    {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_details":
                            ...
                            return false;
                        case "time_expired":
                            MessageBox.Show("Kullanım Süreniz Doldu!", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "hwid_updated":
                            MessageBox.Show("Hwidiniz Kayıt Altına Alındı Lütfen Tekrar Giriş Yapın", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "invalid_hwid":
                            MessageBox.Show("Bu kullanıcı başka bir bilgisayara bağlı, lütfen destek ekibiyle iletişime geçin!", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                    }
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_details":
                            MessageBox.Show("Verilen Bilgiler Yanlış", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "time_expired":
                        ...
                        case "hwid_updated":
                            ...
                            return false;
                        case "invalid_hwid":
                            MessageBox.Show("Bu kullanıcı başka bir bilgisayara bağlı, lütfen DemirPremiumla İletişime Geçin", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                    }
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_license":
                            MessageBox.Show("Lisansı Bulamadık", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "email_used":
                            MessageBox.Show("Email Zaten Kullanılıyor!", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_token":
                            MessageBox.Show("Token Bulunamadı!", ApplicationSettings.Name);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "invalid_details":
